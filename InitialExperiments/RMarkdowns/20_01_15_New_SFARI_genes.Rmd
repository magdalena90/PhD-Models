...
# SFARI Genes
SFARI_genes = read_csv('./../Data/SFARI/SFARI_genes_01-03-2020_with_ensembl_IDs.csv')
...
There are 913 genes with a SFARI score. but to map them to gene expression mapa we had to map the gene names to their corresponding ensembl IDs
<br><br>
...
- Since a gene can have more than one ensembl ID, there were some one-to-many mappings between a gene name and ensembl IDs, so that's why we ended up with 1031 rows in the SFARI_genes dataset.

- 31 genes had duplicated IDs, one with the regular **ENSG...n** format and another one with **LRG_n** format. Since all of the genes in our expression dataset have the **ENSG...n** notation, the second notation was removed

- 15 genes weren't assigned any ID by bioMart (ADSS2, BICDL1, DIPK2A, EMSY, ERBIN, H2BC11, KMT5B, LNPK, METTL26, MIR137, MRTFB, MSNP1AS, NEXMIF, NSMCE3, PATJ, PRKN, PTCHD1-AS, RP11-1407O15.2, RPS10P2-AS1, TERB2, TSPOAP1), so the original ensembl ID that came with these genes was used, hoping they don't have any alternative ensembl IDs.

- In the end, all SFARI genes could be mapped to an ensembl ID

**Note:** There are some genes in the SFARI Gene score list that do not have any score assigned to them, and three of them don't have a syndromic tag either. **I'm not sure what's going on with these genes or why they are on the list?**.

```{r}
print(paste0('There are ', sum(is.na(SFARI_genes$`gene-score`)) ,
...
858/913 SFARI genes are present in the dataset (~94%), 
```{r}
print(paste0('There are ', sum(SFARI_genes$ID %in% rownames(datExpr)), ' SFARI Genes in the expression dataset'))

print(paste0('Of these, only ', sum(DE_info$`gene-score`!='None'), ' have an assigned score'))
```

**From now on, we're only going to focus on the 799 genes with a score**
...
- This pattern is generally consistent on all log fold change thresholds, but the three scores are quite similar between them

- All SFARI scores are more affected by the filtering than the genes with Neuronal-related functional annotations

- SFARI gene groups 1 and 2 are more affected than the average gene, especially as the fold change increases

- At a threshold of log2(1.11), we lose all genes belonging to score 1, and at log2(1.13) all the genes belonging to score 2

- If we stick to the original null hypothesis $H_0: lfc=0$, 237/700 SFARI genes are statistically significant (30%)
