...
- Running the model multiple times to get more acurate measurements of its performance no different separation of train and test

- The final model is created averaging the probabilities assigned on each of these runs to the test set (in the end all the genes are in the test set at least 64 times)
...
- 7 SFARI genes remainnone with score), so they still have a higher concentration (1:7) than in the whole test set (1:17)
